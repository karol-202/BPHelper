...
) : TableConfiguration()
{
	...
	) : TableConfiguration.Table
	{
		class Builder(@StringRes val name: Int) : TableConfiguration.Table.Builder<Table>()
		{
			...
			override fun build() = if(freeSeats == 0) Table(
				name,
				members[0],
				members[1]
			) else null
	...
	companion object : Factory<Table>()
	{
		...
		override fun createBuilders() = tableNames.map {
			Table.Builder(
				it
			)
		}
...
) : TableConfiguration()
{
	...
	{
		private val tableNames = listOf(
			R.string.table_name_gov, R.string.table_name_opp
		)
		...
		override fun createFromTables(tables: List<Table>) =
			TableConfiguration2X3(tables[0], tables[1])
	}
}
...
) : TableConfiguration()
{
	...
	companion object : Factory<Table>()
	{
		...
		override fun createFromTables(tables: List<Table>) =
			TableConfiguration2X4(tables[0], tables[1])
	}
}
